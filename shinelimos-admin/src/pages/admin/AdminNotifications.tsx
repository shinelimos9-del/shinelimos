...
export default function AdminNotifications() {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] bg-[#050505]/50 border border-white/10 rounded-2xl overflow-hidden glass-dark text-left">
      ...
      <div className="w-full md:w-1/3 border-r border-white/10 flex flex-col bg-black/40">
        <div className="p-4 border-b border-white/10">
          ...
          <div className="flex items-center bg-[#111] border border-white/10 rounded-xl px-3 py-2 focus-within:border-purple-500/40 transition-all">
          ...
          {filteredNotifications.length > 0 ? (
            filteredNotifications.map(notification => (
              <div 
                ...
                className={`p-4 border-b border-white/5 cursor-pointer transition-colors ${selectedId === notification.id ? 'bg-purple-500/10 border-l-2 border-l-purple-500' : 'hover:bg-white/5 border-l-2 border-l-transparent'}`}
              >
                <div className="flex justify-between items-start mb-1">
                  <h4 className={`text-sm flex items-center gap-2 ${!notification.read ? 'text-white font-semibold' : 'text-white/70'}`}>
                    {!notification.read ? <Mail size={14} className="text-purple-400" /> : <MailOpen size={14} className="text-white/40" />}
                    {notification.title}
                  </h4>
        ...
        {selectedNotification ? (
          <div className="p-8 animate-in fade-in duration-300">
            <div className="flex justify-between items-start border-b border-white/10 pb-6 mb-6">
              ...
              <div className="px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 text-xs font-medium uppercase tracking-wider border border-purple-500/30">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-xl p-5">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-purple-500/20 text-purple-300 flex items-center justify-center text-sm font-bold border border-purple-500/30">
