    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">Manage fleet and pricing.</p>
        </div>
        <button 
          onClick={() => handleOpenModal()}
          className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 shadow-lg shadow-purple-500/20"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {vehicles.map((v) => (
            <div key={v._id} className="glass-dark rounded-2xl border border-white/10 p-4 hover:border-purple-500/30 transition-all hover:scale-[1.02] group flex flex-col relative">
              <div className="bg-black/60 rounded-xl aspect-4/3 flex items-center justify-center p-4 mb-4 overflow-hidden relative group/img border border-white/5">
                 <img 
                 ...
                 <button 
                   ...
                   className="absolute top-2 right-2 bg-black/60 hover:bg-red-600 text-white p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity shadow-lg backdrop-blur-sm"
                 >
                   <Trash2 size={16} />
                 </button>
              </div>
              <div className="text-center flex-1 flex flex-col">
                ...
                <div className="flex flex-col gap-1 mb-4">
                  <div className="inline-block mx-auto px-4 py-1 rounded-full bg-purple-500/20 border border-purple-500/30 text-xs text-purple-300 font-mono">
                ...
                <div className="mt-auto pt-2">
                  <button 
                    ...
                    className="w-full bg-white/10 hover:bg-purple-600 text-white border border-white/10 hover:border-purple-500 transition-colors py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2"
                  >
                     Update Now
                  </button>
                </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 text-left">
          <div className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto no-scrollbar shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-6 border-b border-white/10 flex items-center justify-between sticky top-0 bg-[#111]/90 backdrop-blur-md z-10">
            ...
            <form onSubmit={handleSaveVehicle}>
              <div className="p-6 space-y-6">
                <div 
                  ...
                  className="rounded-2xl bg-white/5 border-2 border-dashed border-white/10 p-8 flex flex-col items-center justify-center text-center transition-colors cursor-pointer group hover:border-purple-500/40"
                >
                  {imagePreview ? (
                    ...
                    <>
                      <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-4 group-hover:bg-purple-500/20 transition-colors">
                        <Upload className="text-white/40 group-hover:text-purple-400" size={20} />
                      </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <label className="border border-dashed border-white/10 hover:border-purple-500/40 bg-white/5 hover:bg-purple-500/10 rounded-xl aspect-[4/3] flex flex-col items-center justify-center cursor-pointer transition-colors group">
                      <Plus className="text-white/40 group-hover:text-purple-400 mb-1" size={16} />
                      <span className="text-[10px] text-white/40 group-hover:text-purple-400 font-medium">Add Image</span>
                      <input
                        type="file"
                        multiple
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="md:col-span-2">
                    ...
                    <input 
                      ...
                      required
                      className="w-full bg-black border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:border-purple-500/60 focus:outline-none transition-colors placeholder:text-white/20" 
                    />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea 
                      ...
                      required
                      rows={3}
                      className="w-full bg-black border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:border-purple-500/60 focus:outline-none transition-colors placeholder:text-white/20 resize-none" 
                    />
                  </div>

                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Features</label>
                    <div className="flex flex-wrap gap-2 p-3 bg-black border border-white/10 rounded-xl min-h-[50px]">
              ...
              <div className="p-6 border-t border-white/10 flex justify-end gap-3 sticky bottom-0 bg-[#111]/90 backdrop-blur-md">
                 <button 
                   type="button"
                   onClick={handleCloseModal} 
                   className="px-6 py-2.5 rounded-xl text-sm font-medium text-white/70 hover:bg-white/10 transition-colors"
                 >
                   Cancel
                 </button>
                 <button 
                   type="submit"
                   disabled={submitting}
                   className="bg-purple-600 hover:bg-purple-700 disabled:bg-purple-600/50 disabled:cursor-not-allowed text-white px-8 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-lg shadow-purple-500/20 flex items-center gap-2"
                 >
